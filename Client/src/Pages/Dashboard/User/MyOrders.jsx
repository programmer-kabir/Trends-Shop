...
// Incomplete Data
  const matchIncompleteData = Booked
  .map((bookedItem) => {
    const matchingShoe = Shoes.find((shoe) => shoe._id === bookedItem.productId);
    if (matchingShoe) {
      return {
        _id: bookedItem._id,
...
// Match Complete Data
const matchCompleteData = Booked
.map((bookedItem) => {
  const matchingShoe = Shoes.find((shoe) => shoe._id === bookedItem.productId);
  if (matchingShoe) {
    return {
...
console.log(matchCompleteData);
 ...
 <p className="text-sm text-gray-900 font-medium">
     ...
     <div className="flex gap-2 items-center">
     {data?.status === "Completed Order" ? <button className=" flex disable items-center gap-2 bg-[#439DDF] text-white rounded font-semibold px-3 py-1">
      Completed Order
      </button>:""}
